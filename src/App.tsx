  ...
  const handleExportExcel = async () => {
    try {
      ...
      const teacherData = records.map(r => ({
        "Project": r.project, "Group Name": r.groupName, "Raw Group Score": r.rawGroupScore, "Weighted Group Score (14%)": parseFloat(r.weightedGroupScore.toFixed(2)), "Avg Peer Score (Raw)": parseFloat(r.avgPeerRaw.toFixed(2)), "Weighted Peer Score (6%)": parseFloat(r.weightedIndivScore.toFixed(2)), "Total Score (20%)": parseFloat(r.totalScore.toFixed(2)), "Peer Voters Count": r.groupEvalsCount, "Evaluation Time": new Date(r.id).toLocaleString('th-TH')
      }));
      const studentData = peerEvaluations.map(p => {
        ...
        return { "Project": p.project, "Group Name": p.groupName, "Evaluator": p.evaluator, "Target": p.target, "Total Score (15)": p.totalRaw, ...scoreDetails, "Evaluation Time": new Date(p.timestamp).toLocaleString('th-TH') };
      });
      ...
      sortedGroups.forEach(group => {
          ...
          group.membersArray.forEach(member => {
              ...
              summaryData.push({ "รหัส": member.id || '-', "ชื่อ-นามสกุล": member.name || member.label, "ชื่อ Project": group.project, "ชื่อ กลุ่ม": group.name, "คะแนนกลุ่ม (14%)": parseFloat(groupScore.toFixed(2)), "คะแนนเพื่อน (6%)": parseFloat(peerScoreWeighted.toFixed(2)), "Total Percent (20%)": parseFloat(totalScore.toFixed(2)) });
          });
      });
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(summaryData.length > 0 ? summaryData : [{"Info": "No data"}]), "Summary");
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(teacherData.length > 0 ? teacherData : [{"Info": "No teacher data"}]), "Teacher_Evaluation");
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(studentData.length > 0 ? studentData : [{"Info": "No peer data"}]), "Student_Peer_Evaluation");
